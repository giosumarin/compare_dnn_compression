...
# Clustering is applied to convolutional (64 clusters) and FC (64 clusters) layers. 
# After compression we calculate space occupancy (psi in paper), using
# Index Map for convolutional layers and HAM for dense layers.
...
# After compression we calculate space occupancy (psi in paper), using
# Index Map for convolutional layers and HAM or sHAM for dense layers.
# Results of compression ratio will be printed on terminal and will be saved in txt format 
# at path experiments/space_performance/results/.
